import { type Color, DEFAULT_ALPHA } from "../model/types";
...
export function hexToRgba(hex: string, alpha: number = DEFAULT_ALPHA): Color {
  ...
  const r = Number.parseInt(isShortHex ? hexCode[0] + hexCode[0] : hexCode.substring(0, 2), 16);
  const g = Number.parseInt(isShortHex ? hexCode[1] + hexCode[1] : hexCode.substring(2, 4), 16);
  const b = Number.parseInt(isShortHex ? hexCode[2] + hexCode[2] : hexCode.substring(4, 6), 16);
